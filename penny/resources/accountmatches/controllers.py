...
from penny.resources.accountmatches.forms import (
    FormAccountMatch, FormAccountMatchFilter
)
...
def _accountmatches():
    return render_template(
        'accountmatches.html',
        data_url=url_for('data_accountmatches.accountmatches')
    )
...
def accountmatch(id):
    ...
    try:
        accountmatch = models.db.session.query(models.AccountMatch) \
    ...
    filters = models.db.session.query(models.AccountMatchFilterRegex) \
    ...
    if form.validate_on_submit():
        ...
        if 'update' in request.form:
            ...
            return render_template('accountmatch.html', filters=filters,
    ...
    return render_template('accountmatch.html', form=form, filters=filters,
                           accountmatch=accountmatch)
...
def add():
    ...
    if form.validate_on_submit():
        ...
        if form_filter and form_filter.regex.data:
            ...
            amfr = models.AccountMatchFilterRegex(accountmatch=accountmatch,
                                                  regex=form_filter.regex.data)
            models.db.session.add(amfr)

        models.db.session.add(accountmatch)
        models.db.session.commit()

        return redirect(url_for('accountmatches.accountmatch',
                                id=accountmatch.id))

    return render_template('accountmatch.html', form=form,
