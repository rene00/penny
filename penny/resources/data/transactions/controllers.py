...
data_transactions = Blueprint('data_transactions', __name__,
                              url_prefix='/data/transactions')
...
def transactions():
    ...
    total = models.db.session.query(
        func.count(models.Transaction.id)).filter(
            models.Transaction.user == g.user,
            models.Transaction.is_deleted == 0)

    transactions = models.db.session.query(models.Transaction) \
        .filter(models.Transaction.user == g.user,
                models.Transaction.is_deleted == 0).order_by(
...
@data_transactions.route('/bankaccount/<int:id>',
                         defaults={'start_date': None, 'end_date': None})
@data_transactions.route('/bankaccount/<int:id>/'
...
def bankaccount(id, start_date, end_date):
    ...
    if start_date and end_date:
        total = total.filter(models.Transaction.date >= start_date,
                             models.Transaction.date <= end_date)
...
def accounttype(accounttype, start_date, end_date):
    ...
    results = models.db.session.query(
        ...
        .filter(
            models.Account.accounttype_id == models.AccountType.id
        ) \
        .filter(
            models.AccountType.parent == at
        ) \
