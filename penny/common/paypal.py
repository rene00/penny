...
from penny.common.transactions.create import CreateTransaction
...
def get_transactions(transactions, bankaccount, user):
    ...
    for row in reader:
        ...
        # received funds
        if (
            paypal.type == "Payment received"
            or paypal.type == "Web Accept Payment Received"
        ):
            paypal.credit = abs(int(round(float(row[" Gross"].replace(",", "")) * 100)))
            paypal.fee = int(round(float(row[" Fee"].replace(",", "")) * 100))
            ...
            # refund
            paypal.debit = int(round(float(row[" Gross"].replace(",", " ")) * 100))
            # refund fee is a debit and must be negative
            paypal.fee = int(round(float(row[" Fee"].replace(",", "")) * 100)) * -1
            paypal.email = row[" From Email Address"]
            paypal.name = row[" Name"]
            paypal.memo = "{} - {}".format(paypal.type, paypal.name)
            ...
            paypal.debit = int(round(float(row[" Gross"].replace(",", "")) * 100))
            paypal.fee = abs(int(round(float(row[" Fee"].replace(",", "")) * 100)))
            ...
            # cancelled fee
            paypal.credit = int(round(float(row[" Gross"].replace(",", "")) * 100))
            paypal.memo = paypal.type
            ...
            paypal.credit = abs(int(round(float(row[" Net"].replace(",", "")) * 100)))
            if row[" Currency"] == "AUD":
                paypal.memo = "{} - {}".format("Payment Received", paypal.type)
